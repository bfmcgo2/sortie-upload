...
import { mkdtempSync, existsSync, accessSync, constants } from 'fs';
...
// Use require() instead of import to ensure binaries are included in serverless bundle
let actualFfmpegPath, actualFfprobePath;

try {
  // Try to get the paths - these will be resolved at runtime
  const ffmpegStatic = require('ffmpeg-static');
  const ffprobeStatic = require('ffprobe-static');
  
  actualFfmpegPath = typeof ffmpegStatic === 'string' ? ffmpegStatic : ffmpegStatic.default || ffmpegStatic;
  actualFfprobePath = ffprobeStatic.path || ffprobeStatic.default?.path || ffprobeStatic;
  
  // Verify binaries exist and are accessible
  if (!actualFfmpegPath || !existsSync(actualFfmpegPath)) {
    console.error('FFmpeg binary not found at:', actualFfmpegPath);
    throw new Error(`FFmpeg binary not found. Expected at: ${actualFfmpegPath}`);
  }
  
  if (!actualFfprobePath || !existsSync(actualFfprobePath)) {
    console.error('FFprobe binary not found at:', actualFfprobePath);
    throw new Error(`FFprobe binary not found. Expected at: ${actualFfprobePath}`);
  }
  
  // Check if binaries are executable
  try {
    accessSync(actualFfmpegPath, constants.X_OK);
    accessSync(actualFfprobePath, constants.X_OK);
  } catch (e) {
    console.warn('Binary permission check failed, but continuing:', e.message);
  }
  ...
  console.error('Failed to load ffmpeg/ffprobe binaries:', error);
  throw new Error(`FFmpeg/FFprobe binaries not available: ${error.message}`);
}
