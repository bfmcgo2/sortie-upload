...
export async function compressVideoForMobile(inputPath, outputPath) {
  ...
  console.log('Compressing video for mobile optimization...');
  
  try {
    // Mobile-optimized compression: lower resolution, bitrate, and quality
    const command = `"${actualFfmpegPath}" -i "${inputPath}" -vf "scale=720:-2" -c:v libx264 -preset fast -crf 28 -c:a aac -b:a 128k -movflags +faststart "${outputPath}"`;
    console.log('Mobile compression command:', command);
    
    await execAsync(command);
    console.log('Mobile video compression completed');
    return { success: true };
  } catch (error) {
    console.error('Mobile video compression failed:', error);
    return { success: false, error: error.message };
  }
}
